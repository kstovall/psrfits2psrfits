...
usage(void)
{
  fprintf(stderr,"%s","   [-b numbits] -o outfile [-g numgb] [--] infiles ...\n");
  ...
  fprintf(stderr,"%s","         -g: The output file size in GB\n");
  fprintf(stderr,"%s","             1 float value between 0.1 and 100.0\n");
  fprintf(stderr,"%s","             default: `10.0'\n");
  fprintf(stderr,"%s","    infiles: Input PSRFITS file names (in time order)\n");
  fprintf(stderr,"%s","             1...100 values\n");
  fprintf(stderr,"%s","  version: 31May10\n");
  fprintf(stderr,"%s","  ");
  exit(EXIT_FAILURE);
}
/**********************************************************************/
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-g", argv[i]) ) {
      int keep = i;
      cmd.numgbP = 1;
      i = getFloatOpt(argc, argv, i, &cmd.numgb, 1);
      cmd.numgbC = i-keep;
      checkFloatLower("-g", &cmd.numgb, cmd.numgbC, 100.0);
      checkFloatHigher("-g", &cmd.numgb, cmd.numgbC, 0.1);
      continue;
    }
